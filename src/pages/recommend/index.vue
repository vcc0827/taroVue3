            ...
            <scroll-view class="card" :scroll-y="true" :scroll-into-view="scrollIntoView" @scroll="scroll"
              ...
              <view class="info">
                <view class="base__info">
                  ...
                  <view class="info-item" v-if="recommend.living_place">现居地：
                    <text>{{ recommend.living_place }}</text>
                  </view>
                  <view class="info-item" v-if="recommend.hometown">家乡：
                ...
                <view class="identify">
                  ...
                  <view class="identify-box">
                    <view class="identify-id-box"
                      :style="{ background: `${recommend.is_real_name_auth ? '#D9FFDE' : '#F8F8F8'}` }">
                      <image lazy-load
                        :src="recommend.is_real_name_auth ? getImageUrl('https://static.hamu.site/mini/identify-id_yes.png', 64, 64) : getImageUrl('https://static.hamu.site/mini/identify-id_no.png', 64, 64)" />
                    ...
                    <view class="identify-real-box"
                      :style="{ background: `${recommend.is_real_name_auth ? '#D9F0FF' : '#F8F8F8'}` }">
                      <image lazy-load
                        :src="recommend.is_real_name_auth ? getImageUrl('https://static.hamu.site/mini/identify-real_yes.png', 64, 64) : getImageUrl('https://static.hamu.site/mini/identify-real_no.png', 64, 64)" />
                ...
                <view class="photos" v-if="recommend.img_list.length">
                  ...
                  <view v-for="(item, index) in recommend.img_list" :key="index" class="info-item">
                    <image class="recommend__image_list" mode="aspectFit" :src="getImageUrl(item, 610, 610)" lazy-load
                ...
                <view class="ideal-lover"
                  v-if="recommend && recommend.ideal_object && (recommend.ideal_object['理想对象介绍'] || (recommend.ideal_object_tags && recommend.ideal_object_tags.length))">
                  ...
                  <view class="privacy__list">
                    <block v-if="recommend.private_info">
                      <view class="privacy__item" v-for="([key, value], index) in filterPrivateInfo" :key="index">
                        ...
                        <view class="privacy__info">{{ key === '年收入' ? value + '万' : value }}
                        </view>
                      </view>
                      <view class="privacy__item bought">
                        ...
                        <view class="privacy__info">
                          ...
                          <image v-if="recommend.is_buy" src="https://static.hamu.site/mini/detail/copy.png"
                ...
                <view v-for="(person, index) in recommend" class="recommend__person" :key="person.id">
                  <view @tap="navigateToCustomer(person.id)" style="width: 100%">
                    ...
                    <view class="specific__info">
                      ...
                      <view>
                        {{ person.height }}cm
                        <text space="nbsp" class="ellip">{{ person.position && person.position.length ?
                          person.position.join("") : person.job }}</text>
                      </view>
                    </view>
                  </view>
                </view>
            ...
            <view class="card empty__state">
              ...
              <block wx:else>
                <view class="subscribe">
                  ...
                  <image
                    :src="getImageUrl('https://static.hamu.site/mini/home/match_official_account_qrcode.png', 270, 270)"
                    class="qrcode" :show-menu-by-longpress="true" lazy-load />
                </view>
...
const filterPrivateInfo = computed(() => {
  return Object.entries(recommends.value[current.value].private_info).filter(([key, value]) => {
    return !(key === 'QQ号' || key === '其他经济条件' || key === '微信号' ||
      key === '是否有房' || key === '是否有车' || key === '是否未购') &&
      value &&
      !(key === '年收入' && value === '0');
  });
})
...
const handleRecommends = async () => {
  ...
  if (recommends.value) {
    recommends.value.forEach((item) => {
      ...
      if (ideal['年收入']) {
        item.ideal_object['收入'] = `${ideal['年收入']}`.indexOf('万') > -1
          ? ideal['年收入']
          : ideal['年收入'] + '万';
        delete item.ideal_object['年收入'];
      }
      if (ideal['身高'] && `${ideal['身高']}`.indexOf('cm') === -1) {
        item.ideal_object['身高'] =
          ideal['身高'] === '不限'
            ? ideal['身高']
            : ideal['身高'] + 'cm';
      }
      ...
      .empty__state {
        ...
        .recommend__content {
          ...
          .recommend__person {
            ...
            box-shadow: 0px 0px 28rpx 0px rgba(0, 0, 0, 0.06);
            border-radius: 10rpx;
